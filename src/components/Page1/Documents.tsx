...
export default function Documents({ data }: DocumentsProps) {
  ...
  return (
    <div>
      <section className="max-w-6xl mx-auto py-12 px-4 md:px-8" id="documents">
        <h2 className="text-2xl md:text-3xl font-bold text-center mb-2">How to Apply for GST Registration Online?</h2>
        <p className="text-center text-gray-600 mb-10">The step-by-step guide to apply for GST registration:</p>
        <div className="flex flex-col md:flex-row items-start gap-10">
          <div className="flex-1">
            <img src="https://corpbiz.io/img/steps_img.webp" alt="Form Fill" className="rounded-xl w-full object-cover shadow-lg" />
          </div>
          <div className="flex-1 space-y-6">
            {steps.map((step, idx) => (
              <div key={idx} className="border-b pb-4">
                <div className="flex gap-4">
                  <div className="font-semibold text-blue-600 italic min-w-[40px]">{idx + 1}.</div>
                  <div>
                    <h3 className="font-semibold italic text-gray-800">{step.title}</h3>
                    <p className="text-gray-600 mt-1">{step.desc}</p>
                  </div>
      ...
      <section className="bg-white py-12 px-4 md:px-8">
        <div className="max-w-6xl mx-auto text-center mb-10">
          <h2 className="text-2xl md:text-3xl font-bold mb-2">Consequences for Non-Compliance with GST Registration</h2>
          <p className="text-gray-600">Violations attract the following penalties:</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          <div className="bg-blue-100 p-6 rounded-lg shadow-md">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Penalty For Violation In Case Of GST Registration</h3>
            <ul className="space-y-3">
              {violations.map((v, i) => (<li key={i} className="flex items-start gap-2"><span className="text-orange-500 mt-1">✔️</span><span className="text-gray-800">{v}</span></li>))}
            </ul>
          </div>
          <div className="bg-blue-800 text-white p-6 rounded-lg shadow-md">
            <h3 className="text-lg font-bold mb-4">Consequences For Not Registering GST</h3>
            <ul className="space-y-3">
              {consequences.map((c, i) => (<li key={i} className="flex items-start gap-2"><span className="text-orange-400 mt-1">✔️</span><span>{c}</span></li>))}
            </ul>
          </div>
        </div>
      </section>
