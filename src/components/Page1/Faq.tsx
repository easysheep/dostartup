...
export default function ServicesFaq({ services, faqs }: ServicesFaqProps) {
  ...
  return (
    <div>
      <section className="bg-white py-16 px-4 max-w-7xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-semibold text-center mb-12">Why Trust Corpbiz for GST Registration Certificate?</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((srv, idx) => (
            <div key={idx} className="border rounded-lg p-5 shadow-sm hover:shadow-md transition">
              <div className="mb-4"><img src="https://corpbiz.io/img/adv_icon1.webp" alt="Icon" className="w-10 h-10" /></div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        <h2 className="text-2xl md:text-3xl font-semibold text-center">Frequently Asked Questions on GST Registration</h2>
        <p className="text-center text-gray-500 mt-2 mb-10">Have a look at the answers to the most asked questions.</p>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          <div className="md:col-span-4 flex justify-center"><img src="https://corpbiz.io/img/adv_icon1.webp" alt="FAQ Illustration" className="w-64" /></div>
          <div className="md:col-span-8">
            {faqs.map((faq, idx) => (
              <div key={idx} onClick={() => setActiveIndex(activeIndex === idx ? null : idx)} className="border-b py-4 cursor-pointer transition-all">
                <div className="flex justify-between items-center">
                  <h3 className="text-base font-medium">{faq.question}</h3>
                  {activeIndex === idx ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                </div>
                {activeIndex === idx && <p className="mt-2 text-sm text-gray-600">{faq.answer}</p>}
              </div>
            ))}
          </div>
        </div>
