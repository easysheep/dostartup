...
export default function Features({ items }: FeaturesProps) {
  return (
    <section id="features" className="bg-white py-12 px-4 md:px-8">
      <div className="max-w-6xl mx-auto text-center mb-10">
        <h2 className="text-2xl md:text-3xl font-bold mb-2">Key Components of GST Registration</h2>
        <p className="text-gray-600 max-w-2xl mx-auto">The structure of GST registration in India evolves around the major components below:</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {items.map((item, idx) => (
          <div key={idx} className="border rounded-lg p-6 text-left hover:shadow-lg transition-all duration-300">
            <div className="text-4xl mb-4">{item.icon}</div>
            <h3 className="text-md font-semibold mb-2">{item.title}</h3>
            <p className="text-gray-700 text-sm">{item.description}</p>
          </div>
